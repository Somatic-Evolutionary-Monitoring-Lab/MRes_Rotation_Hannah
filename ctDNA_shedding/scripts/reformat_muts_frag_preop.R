...
######   - Including preoperative samples                                 ######
######   - Add CRUK ID                                                    ######
...
# Read in table - this contains the ECLIPSE processed data from the patients from Black et al 2025 Cell
...
# Keep all clonal, ctDNA positive timepoints at high ppm
ctDNA_data_clonal_presurg <- ctDNA_data %>%
  filter(!is.na(mutation_cluster), 
...
n_patients_presurg <- length(unique(ctDNA_data_clonal_presurg$patient))

# -----------------------------------------------------------------------------
# Calculate CCF z-scores
# -----------------------------------------------------------------------------

ctDNA_data_clonal_presurg <- ctDNA_data_clonal_presurg %>%
...
liftover_input <- ctDNA_data_clonal_presurg %>%
  mutate(
...
write_tsv(
  liftover_input,
  file      = "data/ctDNA_data_clonal_presurg_black_mutations_hg19_unique.bed",
  col_names = FALSE
)


# -----------------------------------------------------------------------------
# Do liftOver at https://genome.ucsc.edu/cgi-bin/hgLiftOver from hg19 to hg38
# -----------------------------------------------------------------------------
...
liftover_output <- read_tsv(
  "data/ctDNA_data_clonal_presurg_black_mutations_hg38_unique.bed",
...
ctDNA_data_hg38 <- ctDNA_data_clonal_presurg %>% 
  mutate(
    chrom = paste0("chr", chromosome),
    start = position - 1,
    end   = position,
...
write_tsv(
  ...
  paste0("data/ctDNA_data_clonal_preop_black_hg38_ccfzscore_highlow_", format(Sys.Date(), "%Y%m%d"), ".tsv")
)
