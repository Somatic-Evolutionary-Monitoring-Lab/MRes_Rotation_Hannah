...
# Check the overlap
unique(ctDNA_data$patient_name) %in% clinical$Shorter_ID

# Merge
ctDNA_clinical <- ctDNA_data %>%
  left_join(clinical, by = c("patient_name" = "Shorter_ID"))


###############################################################
#### Summarise to patient level — mean of mutation means   ####
###############################################################

patient_level <- mutation_level %>%
